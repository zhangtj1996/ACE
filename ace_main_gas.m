...
%This function calculates the conditional expectation without smoothing.
%x=[y x1 x2 ...]
...
while oi1<=oi && ocrit1>ocrit
    ...
    while ii1<=ii && icrit1>icrit
        for d=2:dim+1; sum0=0;
            ...
            phi(d,:)=cef(phi(1,:)-sum0,x(d,:));%cef(phi(1,:)-sum0,ind(d,:),ranks(d,:),wl,ll); %generate phi
        end;
        icrit1=ieps;
        if dim==1; sum0=phi(2,:); else sum0=sum(phi(2:dim+1,:)); end;
    ...
    phi(1,:)=cef(sum0,x(1,:));%cef(sum0,ind(1,:),ranks(1,:),wl,ll);
...
function A=cef(theta,X)
A=zeros(length(X),1);
t=unique(X);
for i=1:length(unique(X))
    A(find(X==t(i)))=mean(theta(find(X==t(i))));
end
